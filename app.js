...
const baseOptions = (axisTitle) => ({ responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: '#c9daf5' } }, tooltip: { mode: 'index', intersect: false } }, scales: { x: { ticks: { color: '#97a9c7', maxTicksLimit: 7 }, grid: { color: '#183854' } }, y: { ticks: { color: '#97a9c7' }, grid: { color: '#183854' }, title: { display: true, text: axisTitle, color: '#97a9c7' } } } });
...
$('form').addEventListener('submit', async (event) => {
  ...
  $('status').textContent = 'Fetching live prices, macro context, and calculating next-day forecast...';
  ...
  try {
    ...
    $('change').textContent = `${data.expected_change_pct >= 0 ? '+' : ''}${data.expected_change_pct}% estimated change from latest close`;
    ...
    $('status').textContent = `${data.symbol} | data as of ${data.as_of} | ${data.cached ? 'served from 30-minute cache' : 'fresh model run'}`;
